...
class EtherScanIoApi(object):
    ...
    def get_contracts(self, start=0, end=None):
        ...
        while not end or page <= end:
            for _ in range(20):
                resp = self.session.get("/contractsVerified/%d?filter=opensourcelicense" % page).text
                if "Sorry, You have reached your maximum request limit for this resource." in resp:
                    print("[[THROTTELING]]")
                    time.sleep(1+2.5*_)
                    continue
                else:
            ...
            page, lastpage = re.findall(r'Page <strong class="font-weight-medium">(\d+)</strong> of <strong class="font-weight-medium">(\d+)</strong>', resp)[0]
            ...
            for col in rows:
                contract = {'address': self._extract_text_from_html(col[0]).split(" ",1)[0],
                               ...
                               'version': self._extract_text_from_html(col[3]),
                               ...
                               'license': self._extract_text_from_html(col[8]),
                               }
                if contract['version'].startswith('0.5.') and contract['txcount'] > 10000:
                    yield contract
            page += 1
    ...
    def get_contract_source(self, address):
        ...
        for _ in range(20):
            ...
            driver.get("https://etherscan.io/address/%s#code"%address)
            ...
            try:
                ...
                return_value =  driver.execute_script("return editor.getValue()")
                driver.close()
                return return_value
